...
As pointed out by Weiskopf et al., there's already a lot of research around uncertainty visualization. 
but as pointed out by Leffrang et al. there's few research on uncertainty on time series forecasting, and more 
specifically on probability estimation: not knowing the overall uncertainty, but to actually estimate probabilities.
This would help decision takers estimate actual probabilities derived from their decisions.

In this article, we will present the related work on uncertainty visualization and how to apply it to 
the probability estimation case for Probabilistic time series forecasting models. 
Then we will compare those methods against the question: 
how well can we estimate probabilities with it. 
