...
median of the distribution
 over time alongside 2 lines: the evolution of the 5\ts{th} and 95\ts{th} percentile of the distribution. 
...
at the 25\ts{th} and 75\ts{th} percentile respectively, a narrower box upper and lower parts show 
the 5\ts{th} and 95\ts{th} percentile and finally a line’s edges show the 1\ts{st} and 99\ts{th} percentile. 
...
delimited by 2 lines: the 1\ts{st} and 99\ts{th} percentile. This representation allows having the
...
step, 10 percentiles from 5\ts{th} to 95\ts{th} (5\ts{th}, 10\ts{th}, 15\ts{th}, ...) are represented: this way, we can estimate the percentage 
...
Note that the 1\ts{st}, 2\ts{nd} and 4\ts{th} questions are requesting the probability the target variable is in a closed interval, whereas
the 3\ts{rd} and 5\ts{th} questions are requesting it for an open interval (value greater than). 
...
Table \ref{table:resultsperquestion} shows the error per question and interestingly we can see that the open interval questions (3\ts{rd} and 5\ts{th} questions) 
had fewer errors than the other questions. The 4\ts{th} question's error rate is high as the distribution of the target 
