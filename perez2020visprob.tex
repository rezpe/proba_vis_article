...
Taking uncertainty into account is shown to be beneficial when taking decisions. 
For example Pinson mentioned how we can optimize the use of wind energy forecasts with 
uncertainty.  In air quality forecasting, Perez-Vasseur et al. showed how probabilistic forecasting 
enables the estimation of the probability the No2 pollutant will reach a certain threshold and become a danger 
to the city inhabitants. Another example is the simulation where they gave uncertain estimations to participants and they were 
getting more revenues than the others without uncertainty. 
Rouston et al. 
...
Also, people already think in terms of uncertainty and they expect it for forecasts: for example, for weather forecast 
they already think in terms of temperature ranges or words which covert a range (warm or cold). As stated by Joslyn et al, 
those forecasts with uncertainty can validate or correct the invalid 
...
can be perceived as confusing. As a result, many predictions are displayed as deterministic. Neverthless, this 
prevents people from taking decision with uncertainty in mind.  
...
that uncertainty understanding should be linked to the way it is communicated. There is a whole research effort to 
improve the presentation of uncertainty so it can be used by people for their decisions.
...
Padilla et al. has listed all the methods for visualizing uncertainty. There's a lot of literature for 
waiting the bus, forecast view, new types of visualizations. However, there's few research on time series forecasting, 
and more specifically on time series forecasting: how can we present the forecast of probabilistic time 
series for easy value reading: not knowing the overall uncertainty, but to actually estimate probabilities.
This would help decision takers estimate actual probabilities derived from their decisions.

Therefore, in this article, we will list all the researched ways to visualize and present uncertainty, 
