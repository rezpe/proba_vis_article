...
This uncertainty also affects machine learning models, whose estimations are naturally 
subject to some degree of uncertainty. One way to present this uncertainty is to provide 
a full probability distribution as an estimation instead of a single value. Probabilistic 
time series forecasting models are an example of this and they provide the full probability distribution 
of the future values of a time series.

Decision making with those kinds of estimations is proven to provide higher rewards and correct invalid biases.

1 People expect uncertainty

However, most of the time, those predictions are shown as deterministic.

Indeed, there is a concern that people will not be able to understand predictions that come with uncertainty. 
This could be due to the abstract nature of the probability concepts or because the presentation of this uncertainty
is confusing. 

However people are already used to deal with uncertainty when they use weather forecasts, for example. 
As such, displaying the uncertainty of machine learning predictions can actually be more helpful than presenting 
a single-value forecast. This is because people are already accustomed to thinking in terms of ranges, 
and so a range forecast from a machine learning model can help them to refine their own predictions.



Forecast 

For example, weather forecast estimations do present uncertainty and people are as noted by Joslyn et al  

, people may have well formed 
expectations based on prior experiences, and those expectations already include some sort of uncertainty: like 
expecting a range of temperatures, instead of a single value. Therefore, they also expect the forecasting models
to have that uncertainty. 
As stated by Padilla et al.,
taking it into account is essential to estimate the potential risks and rewards derived from the decisions. 
Also the unpredictability of the future makes us expect some sort of uncertainty 
in the predictions. As stated by Joslyn et al, those forecasts with uncertainty can validate or correct the invalid 
biases from their expectations.
Rouston et al. 
\cite{roulston_laboratory_2006} showed that analysts improved their rewards when taking decisions from temperature forecasts with uncertainty.

Probabilistic forecast provides a full probability distribution instead of a single value and it's a much 
better approximation to uncertainty.
...
Therefore, inspired by Brenner et al., we propose a social experiment where we will compare different charts 
