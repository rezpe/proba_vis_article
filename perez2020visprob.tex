...
\cite{cota_importance_2014} mentions how visualization are playing an important role in the business 
environment where all the analytical tools are embracing visualization to better understand large quantities of data. Indeed, as 
...
The box plot chart shows different percentile with “boxes”. For each point in time, a 
...
proven \cite{cleveland_graphical_1984} that color is not 
...
The application of frequency framing for time series uncertainty visualization is novel and this chart has 
...
the accuracy of 
the answer. The user is asked to do a self assessment on those metrics at the end of the survey.
...
We then directed the participants to a web site we implemented for this experiment: Each individual is presented a 
unique type of chart from the 4 presented above with an explanation on how it works, 
then when the user considers himself ready, he is asked to estimate 5 probabilities at 5 different moments from the chart. 
The 5 questions are:
\begin{itemize}
...
choice and check if providing interactivity to modify those would improve or worsen the user's reading ability. 
...
dealing with the uncertainty in time series data. All the code is based in the altair python library 
\cite{vanderplas2018altair} and 
