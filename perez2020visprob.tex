...
at the 25\ts{th} and 75\ts{th} percentile respectively, a narrower box upper and lower part shows 
the 5\ts{th} and 95\ts{th} percentile and finally a line’s edge shows the 1\ts{st} and 99\ts{th} percentile. 
...
delimited by 2 lines: the 1\ts{st} and 99\ts{th} percentile. This representation allows to have the
...
the 3\ts{rd} and 5\ts{th} question are requesting it for an open interval (value greater than). 
...
had less errors than the other questions. The 4\ts{th} question's error rate is high as the distribution of the target 
