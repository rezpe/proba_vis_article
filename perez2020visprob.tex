...
(As noted by Joslyn et al. \cite{joslyn_communicating_2010},the shortcomings in uncertainty visualization have lead to a new surge of 
research in visualization techniques. Also Van 
der Bles et al. \cite{van_der_bles_communicating_nodate} state that there is more than 
...
visualizations. Following uncertainty visualization surveys were based on data types (Potter et al. (2011) and Brodlie et al. (2012)), 
...
explain to the reader that there's no one size-fits-all solution and the visualization technique must be chosen according to the use case.
...
possible, only less probable. Also, Belia et al \cite{belia_researchers_2005} showed that even leading researchers had 
trouble reading error bars and they confused 
...
frequency framing. It it true that as stated by van Wijk,
 data visualization can be considered an art and therefore subject to criteria like taste or design aesthetics. 
However, we can evaluate the performance of those visualizations on probability readability.
