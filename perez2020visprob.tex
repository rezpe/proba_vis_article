...
grouped the answers per type of chart, then we built figure \ref{figure:errors} 
which shows 
the distribution of the reading error per type of chart, the confidence and the perceived difficulty. 

Although we selected users with master level, we still did received answers with probabilities 
...
We can see a ranking of visualizations in figure \ref{figure:errors}, where the worst chart in terms of readability is the 
gradient chart and the best is the natural frequency time chart as it has a much lower reading error then the other charts. As stated before, 
...
as box plots display more percentiles than the confidence interval, and shows that if we had to chose between those 2 types of charts, confidence 
interval should be chosen as it is simpler to build. On the other hand, natural 
...
Confidence and perceived difficulty charts (also shown on figure \ref{figure:errors}) mirror the results in the reading errors chart.
However, we see less confidence for the gradient chart.
also show the superior ease of use 
